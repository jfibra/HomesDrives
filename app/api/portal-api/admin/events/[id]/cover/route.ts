...
export async function POST(
  ...
  try {
    ...
    const extension = file.name.includes('.')
      ...
      : ''
    const fileName = `${crypto.randomUUID().slice(0, 8)}-${slugPrefix}${extension}`
    ...
    const coverImageUrl = buildPublicImageUrl(bucketName, storagePath)
    const event = await updatePortalEvent(id, { coverImageUrl })
